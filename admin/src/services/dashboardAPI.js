...
function normalizeRide(row = {}) {
  return {
    ...
    // New schema: ride_type replaces is_shared
    rideType: row.ride_type ?? row.rideType ?? 'solo',
    isShared: (row.ride_type === 'shared') || Boolean(row.is_shared ?? row.isShared),
    isScheduled: Boolean(row.is_scheduled ?? row.isScheduled),
    scheduledAt: row.scheduled_at ?? row.scheduledAt ?? null,
    totalPassengers: toNumber(row.total_passengers ?? row.totalPassengers),
    createdAt: row.created_at ?? row.createdAt ?? null,
    updatedAt: row.updated_at ?? row.updatedAt ?? null,
  };
}
...
function normalizeStats(row = {}) {
  return {
    ...
    scheduledRides: toNumber(row.scheduled_rides),
...
function normalizeAnalytics(payload = {}) {
  ...
  return {
    ...
    live: {
      ...
      scheduledRides: toNumber(payload.live?.scheduled_rides),
