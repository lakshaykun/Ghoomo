...
export function normalizeDriverCandidateRequest(row = {}) {
  ...
  return {
    ...
    isShare: row.ride_type === 'shared' || Boolean(row.is_shared),
    rideType: row.vehicle_type || row.ride_type || 'auto',
    isScheduled: Boolean(row.is_scheduled),
    scheduledAt: row.scheduled_at || null,
    totalPassengers: toNumber(row.total_passengers, 0),
    requestedDrivers: [],
    driver: null,
    otp: null,
...
export function normalizeRide(row = {}, overrides = {}) {
  ...
  return {
    ...
    distance: distanceKm,
    durationMinutes: estimateDurationMinutes(distanceKm),
    // isShare: true when ride_type = 'shared' (new schema) or legacy is_shared
    isShare: row.ride_type === 'shared' || Boolean(row.is_shared ?? overrides.isShare),
    rideType: row.vehicle_type || row.ride_type || overrides.rideType || 'auto',
    isScheduled: Boolean(row.is_scheduled ?? overrides.isScheduled),
    scheduledAt: row.scheduled_at || overrides.scheduledAt || null,
    // Passenger count from ride_participants aggregate
    totalPassengers: toNumber(row.total_passengers, overrides.totalPassengers ?? 0),
    maxPassengers: toNumber(row.min_vehicle_capacity_allowed ?? row.vehicle_seats_snapshot, 0),
